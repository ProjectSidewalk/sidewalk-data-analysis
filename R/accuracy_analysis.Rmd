...
worker.type.single.user.clust.trellis.boxplot <- function(accuracy.type) {
  ...
  n.obs <-
    worker.type.single.user.clust %>%
    group_by(worker.type, label.type) %>%
    summarise(n1 = sum(!single.user.clust & !is.na(!!sym(accuracy.type))),
              n2 = sum(single.user.clust & !is.na(!!sym(accuracy.type))))
...
single.user.clust.ramp.counts <-
  ...
  mutate(n.labels = 100 * n.labels / max(n.labels)) %>%
  ungroup() %>%
  filter(single.user.clust == TRUE)

ggplot(data = single.user.clust.ramp.counts, aes(x = label.type, y = n.labels)) +
...
ggplot(data = single.user.clust.turk.ramp.counts,
       aes(x = as.factor(n.workers), y = n.labels)) +
  geom_col() +
  theme.histogram +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 100)) + # removes weird white space at bottom of plot
...
_Expected_: Confidence intervals (variance..?) will be _slightly larger_ for turkers with majority vote: fewer individual labels -> fewer labels that pass majority vote -> more conditions where turkers couldn't agree on any labels
