...
# library(rmarkdown)
# render('R/accuracy_analysis.Rmd', c('html_document', 'github_document'))
```
...
data.with.raw.accuracy <-
  ...
  filter(label.type != 'NoSidewalk' | granularity == 'street.binary')

# Gets the count of GT labels so we can include them as labels in graphs below.
label.counts <-
  accuracy.data %>%
  filter(!binary, !is.turker, granularity == 'street', !remove.low.severity, single.user.clust) %>%
  group_by(label.type) %>%
  summarize(count = sum(true.pos, false.neg))
gt.label.count.lookup <- setNames(label.counts$count, label.counts$label.type)

# Helper function that is passed, it generates labels that show GT label counts.
...
# as a label in the top-left corner of each boxplot, along with N for each box.
worker.type.trellis.boxplot <- function(accuracy.type) {
  
  # Get the counts of workers with non NA accuracy for this type
  non.na.worker.counts <-
    worker.type.accuracy %>%
    group_by(granularity, label.type) %>%
    summarise(n.anon = sum(worker.type == 'anon' & !is.na(!!sym(accuracy.type))),
           n.reg = sum(worker.type == 'reg' & !is.na(!!sym(accuracy.type))),
           n.turk = sum(worker.type == 'turk' & !is.na(!!sym(accuracy.type))),
           n.maj = sum(worker.type == 'maj' & !is.na(!!sym(accuracy.type))))
  
  # Run ANOVA, and format p-value appropriately for visualizing on the plot. Labels are of the form
  # 'p=x.xxx** n=(n1,n2,n3,n4)
  pval <-
    worker.type.accuracy %>%
    group_by(granularity, label.type) %>%
    do(model = aov(as.formula(paste0(accuracy.type, ' ~ worker.type')), data = .)) %>%
    mutate(p = summary(model)[[1]][["Pr(>F)"]][1]) %>%
    inner_join(non.na.worker.counts, by = c('granularity', 'label.type')) %>%
    mutate(p.str = paste0("p=", sprintf("%.3f", p), sig.sym(p),
                          ' n=(', n.anon, ',', n.reg, ',', n.turk, ',', n.maj, ')'))
...
vote.type.trellis.linechart <- function(accuracy.type) {
  ggplot(data =
    ...
    facet_grid(granularity ~ label.type,
               labeller = labeller(label.type = label.count.labeller)) +
    ylim(0, 1) +
    scale_x_continuous(breaks = c(1, 3, 5), limits = c(0.5, 5.5)) +
    # theme_bw() +
    my.theme.discrete.x +
    theme(legend.position = 'top')
}
...
```{r low-severity-label-counts, echo=FALSE, warning=FALSE, fig.height=3, fig.width=8}
...
worker.type.single.user.clust <-
  ...
  filter(granularity == '5_meter',
         !(label.type %in% c('NoSidewalk', 'Problem')),
...
_Observed_:
* The decrease in labels was most pronounced for registered users, followed by turkers, with few anonymous user labels being removed.
* Most of the labels that were removed were actually false positives (good news!). I expect that this is due to situations where there are two nearby problems/features of the same type, and once duplicates from individual users have been removed, the final placement of the clustered labels were more accurate.
* For turkers with majority vote, there was actually an increase in true positives for all label types but Obstacles!
* For individual users, the decreases in CurbRamp, Obstacle, and NoSidewalk labels are more pronounced than for NoCurbRamp or SurfaceProblem labels.

```{r single-user-clustering-label-counts-1, echo=FALSE, warning=FALSE, fig.height=5, fig.width=6}
single.user.clust.counts <-
  data.with.raw.accuracy %>%
  ...
  summarize(all.positives = sum(true.pos, false.pos),
            true.positives = sum(true.pos),
            false.positives = sum(false.pos)) %>%
  ungroup() %>%
  gather(count.type, count, true.positives, false.positives, all.positives, factor_key = TRUE)

counts.no.clust <- single.user.clust.counts %>% filter(single.user.clust == FALSE)
single.user.clust.count.change <- single.user.clust.counts %>%
  filter(single.user.clust == TRUE) %>%
  mutate(change.percentage = 100 * (count - counts.no.clust$count) / counts.no.clust$count,
         old.count = counts.no.clust$count)

count.labs <- single.user.clust.count.change %>%
  group_by(worker.type, count.type) %>%
  summarise(txt = paste0(sum(old.count), ' -> ', sum(count)))

ggplot(data = single.user.clust.count.change, aes(x = label.type, y = change.percentage)) +
  geom_col(aes(fill = label.type), show.legend = FALSE) +
  scale_fill_manual(values = c('green', 'red', 'blue', 'orange', 'grey')) +
  my.theme.discrete.x +
  scale_y_continuous(expand = c(0.02, 0), limits = c(-50, 25)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1.05, vjust = 1.05)) +
  facet_grid(count.type ~ worker.type) +
  geom_label(data = count.labs,  aes(x = 2, y = 20, label = txt),
             size = 2.7, label.padding = unit(0.15, "lines"))
```
...
single.user.clust.turk.ramp.counts <-
  ...
  ungroup() %>%
  filter(single.user.clust == FALSE)
...
_Observed_: In almost all cases, we do see a slight improvement in precision. The outliers are CurbRamp labels, which have a sizable improvement in precision (especially for registered users); anonymous users, whose precision does not seem to have been affected; and SurfaceProblem labels for turkers, where precision actually slightly decreased.
...
_Observed_: For the same reasons that my other expected effects for multiple turkers for majority vote or consensus, expectations are not met here either. Since there aren't actually fewer majority vote labels after single users clustering, differences in variance shown below do not reflect the size of `n`. I don't see any clear patterns below.

```{r single-user-clustering-variance, echo=FALSE, fig.height=6, fig.width=7}
