...
```{r reading-cleaning-data-combined, include=FALSE, cache=TRUE}
...
# Rename Problem label type as ProbVsNoProb
label.type.levs <- levels(accuracy.data$label.type)
label.type.levs[5:6] <- c('ProbVsNoProb', 'SurfaceProb')
levels(accuracy.data$label.type) <- label.type.levs

# Reorder label type levels.
label.type.levs <- c('CurbRamp','NoCurbRamp','Obstacle','SurfaceProb','NoSidewalk','ProbVsNoProb')
accuracy.data$label.type <- factor(accuracy.data$label.type, label.type.levs)

# More setup: remove binary analysis (needs to be fixed), add is.anon.route column, add
...
get.worker.type <- Vectorize(
  function(is.turker, is.anon.route) {
    if (is.turker == TRUE) 'turker'
    else if (is.anon.route == TRUE) 'anon.vol'
    else 'reg.vol'
  }
)
data.with.raw.accuracy <-
  ...
  mutate(worker.type = factor(get.worker.type(is.turker, is.anon.route))) %>%
  mutate(raw.accuracy = (true.pos + true.neg) / (true.pos + true.neg + false.pos + false.neg)) %>%
  mutate(granularity = ordered(granularity, levels = c('5_meter', '10_meter', 'street')))
...
label.counts <-
  data_frame(
    ...
    label.type = c('CurbRamp','NoCurbRamp','Obstacle','SurfaceProb','NoSidewalk','ProbVsNoProb')
    )
  # accuracy.data %>%
...
This section has a series of boxplots that compare performance of anonymous volunteers, registered volunteers, and turkers. Here, turker mean just a single turker (the first turker to complete each set of routes).
...
worker.type.trellis.boxplot <- function(accuracy.type) {
  ggplot(data = data.with.raw.accuracy, aes_string(x = 'worker.type', y = accuracy.type)) +
  geom_boxplot() +
...
```{r worker-types-raw-accuracy, echo=FALSE, fig.height=4, fig.width=10, warning=FALSE}
...
```{r worker-types-recall, echo=FALSE, fig.height=4, fig.width=10, warning=FALSE}
...
```{r worker-types-precision, echo=FALSE, fig.height=4, fig.width=10, warning=FALSE}
...
```{r worker-types-f-measure, echo=FALSE, fig.height=4, fig.width=10, warning=FALSE}
...
```{r worker-types-specificity, echo=FALSE, fig.height=4, fig.width=10, warning=FALSE}
...
volunteer.trellis.histogram <- function(accuracy.type) {
  ggplot(data = volunteer.grouped %>% mutate(mean = mean(accuracy.type, na.rm = TRUE)),
...
```{r volunteer-raw-accuracy, echo=FALSE, fig.height=4, fig.width=10, warning=FALSE}
...
```{r volunteer-recall, echo=FALSE, fig.height=4, fig.width=10, warning=FALSE}
...
```{r volunteer-precision, echo=FALSE, fig.height=4, fig.width=10, warning=FALSE}
...
```{r volunteer-f-measure, echo=FALSE, fig.height=4, fig.width=10, warning=FALSE}
...
```{r volunteer-specificity, echo=FALSE, fig.height=4, fig.width=10, warning=FALSE}
