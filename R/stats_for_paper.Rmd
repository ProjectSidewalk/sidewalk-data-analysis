...
steps.by.user <-
  ...
  mutate(cur.time = min(time[action == 'start']),
         ...
         steps = if_else(has.step & has.end, 8, steps),
                            ...
                            cur.time)) %>%
  summarise_at(vars(steps, end.time, is.turker), first) %>%
  summarise_at(vars(steps, user.id, end.time, is.turker), first) %>%
  group_by(user.id) %>%
...
get.users.remaining <- Vectorize(
  function(data, step, is.turk = NA) {
    ...
    else nrow(filter(data, steps.complete >= step, is.turker == is.turk))
  },
