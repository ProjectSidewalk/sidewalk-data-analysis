...
get.users.remaining <- Vectorize(
  function(data, step) {
    nrow(data[data$steps.complete >= step,])
  },
  vectorize.args = c('step')
)
users.remaining.tutorial <-
  data.frame(step = seq(0, 8)) %>%
  mutate(users.remaining = get.users.remaining(steps.by.ip, step))
...
dropoffs.by.ip <-
  ...
  mutate(steps.complete = case_when(missions.complete >= 8 ~ 9,
...
users.remaining.dropoffs <-
  data.frame(step = seq(0, 9)) %>%
  mutate(users.remaining = get.users.remaining(dropoffs.by.ip, step))
...
n.did.eight.missions <- users.remaining.dropoffs[4,'users.remaining']
...
percent.did.eight.missions <- 100 * n.did.eight.missions / n.finished.tutorial
```
...
Of the `r n.finished.tutorial` people who finished the tutorial, `r n.took.step` took a step (`r format(percent.took.step, digits = 0)`%), `r n.did.one.mission` finished at least one mission afterwards (`r format(percent.did.one.mission, digits = 0)`%), `r n.did.two.missions` completed at least two missions afterwards (`r format(percent.did.two.missions, digits = 0)`%), and `r n.did.eight.missions` did at least eight missions afterwards (`r format(percent.did.eight.missions, digits = 0)`%).
