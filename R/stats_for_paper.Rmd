...
n.labels.post.filter <- n_distinct(filtered.labels$label_id)
n.users.pre.filter <- n_distinct(audits$user_id)
n.users.post.filter <- n_distinct(filtered.audits$user_id)
```

There were `r n.users.pre.filter` users who placed `r n.labels.pre.filter` labels pre-filtering. After filtering, we are left with `r n.users.post.filter` users (`r format(100 * n.users.post.filter / n.users.pre.filter, digits = 3)`%) who placed `r n.labels.post.filter` labels (`r format(100 * n.labels.post.filter / n.labels.pre.filter, digits = 3)`%).
