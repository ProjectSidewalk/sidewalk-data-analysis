...
format.tukey.table <- function(tukey, outcome.var, fixed.var, test.data, ordering = NULL,
  ...
  indices.to.start <-
    tukey.with.vars.split %>%
    dplyr::filter(p < 0.05 | second.index == 1) %>%
  ...
  inner_join(tukey.with.vars.split, indices.to.start, by = 'first.index') %>%
    dplyr::filter(second.index >= start.second.index) %>%
...
filtered.labels <- dplyr::filter(non.tutorial.labels, user_id %in% filtered.speeds$user_id)
kable(
  filtered.labels %>%
    distinct(label_id, .keep_all = TRUE) %>%
    count(label_type_id) %>%
...
filtered.audits <- dplyr::filter(audits, user_id %in% filtered.speeds$user_id, completed == TRUE)
good.user.audits <- dplyr::filter(audits, user_id %in% good.user.list)
n.audits <- nrow(filtered.audits)
n.audits.with.researchers <- nrow(good.user.audits)
n.streets <- n_distinct(filtered.audits$street_edge_id)
n.streets.with.researchers <- n_distinct(good.user.audits$street_edge_id)
...
researcher.ids <-
  speeds %>%
  dplyr::filter(role %in% c('Researcher', 'Administrator', 'Owner')) %>%
  distinct(user_id) %>%
  pull()

bad.non.research.ids <-
  speeds %>%
  dplyr::filter(role %in% c('Registered', 'Anonymous', 'Turker'),
                labels_per_100m < 3.75) %>%
  distinct(user_id) %>%
...
n.bad.non.research <- length(bad.non.research.ids)
n.filtered.users <- n.researchers + n.bad.non.research
n.users.remaining <- n_distinct(filtered.speeds$user_id)

percent.researchers <- base::format(100 * n.researchers / n.users.pre.filter, digits = 3)
percent.bad.non.research <- base::format(100 * n.bad.non.research / n.users.pre.filter, digits = 3)
percent.filtered.users <- base::format(100 * n.filtered.users / n.users.pre.filter, digits = 3)
...
n.labels.bad.non.research <- n_distinct(non.tutorial.labels[non.tutorial.labels$user_id %in% bad.non.research.ids,'label_id'])
n.filtered.labels <- n.labels.researcher + n.labels.bad.non.research
n.labels.remaining <- n_distinct(filtered.labels$label_id)

percent.labels.researcher <- base::format(100 * n.labels.researcher / n.labels.pre.filter, digits = 3)
percent.labels.bad.non.research <- base::format(100 * n.labels.bad.non.research / n.labels.pre.filter, digits = 3)
percent.filtered.labels <- base::format(100 * n.filtered.labels / n.labels.pre.filter, digits = 3)
percent.labels.remaining <- base::format(100 * n.labels.remaining / n.labels.pre.filter, digits = 3)
```

There were `r n.users.pre.filter` users who placed `r n.labels.pre.filter` labels pre-filtering. Researchers accounted for `r n.researchers` of the users (`r percent.researchers`%) and `r n.labels.researcher` of the labels (`r percent.labels.researcher`%). Non-researchers with low labeling frequency accounted for `r n.bad.non.research` of the users (`r percent.bad.non.research`%) and `r n.labels.bad.non.research` of the labels (`r percent.labels.bad.non.research`%). This means that we filtered out a total of `r n.filtered.users` of the users (`r percent.filtered.users`%) and `r n.filtered.labels` of the labels (`r percent.filtered.labels`%), and are left with `r n.users.remaining` of the users (`r percent.users.remaining`%) and `r n.labels.remaining` of the labels (`r percent.labels.remaining`%).


### Label counts by label type and user group (filtered)

```{r public.deployment.filtered.label.counts, echo=FALSE, warning=FALSE}
ordered.label.type.levs <- c('CurbRamp', 'NoCurbRamp', 'Obstacle', 'SurfaceProblem', 'NoSidewalk',
                             'Occlusion', 'Other')
kable(
  filtered.labels %>%
...
nonfiltered.speeds <-
  speeds %>%
  dplyr::filter(role %in% c('Registered', 'Anonymous', 'Turker'), user_id %in% user.list) %>%
  mutate(role = factor(role, levels = c('Anonymous', 'Registered', 'Turker')))
...
plot.dropoffs <- function(users.remaining.data, dropoff.step.labels) {
  ggplot(aes(x = step, y = users.remaining), data = users.remaining.data) +
    geom_line() +
    geom_point() +
...
plot.dropoffs.split <- function(users.remaining.split.data, dropoff.step.labels) {
  ggplot(aes(x = step, y = users.remaining), data = users.remaining.split.data) +
...
step.labels <- c('Start tutorial',
                 ...
                 'Fifth curb ramp',
                 'Finish tutorial')
plot.dropoffs(users.remaining.tutorial, step.labels)
plot.dropoffs.split(users.remaining.tutorial.split, step.labels)
```
...
dropoff.labels <- c('Finish tutorial',
                    ...
                    '9 missions',
                    '10 missions')
plot.dropoffs(users.remaining.post.tutorial, dropoff.labels)
plot.dropoffs.split(users.remaining.post.tutorial.split, dropoff.labels)
```
...
```{r public.deployment.user.dropoff.plot, echo=FALSE, fig.width=6, fig.height=3}
...
plot.dropoffs(users.remaining.all, combined.dropoff.labels)
plot.dropoffs.split(users.remaining.all.split, combined.dropoff.labels)
```
