...
new.gran.names <- c('5_meter', '5_meter.binary', 'street', 'street.binary')
...
granularity.analysis.data <-
  ...
  filter(granularity %in% c('5_meter.binary', 'street.binary')) %>%
  mutate(granularity = droplevels(granularity),
         granularity = factor(granularity,
                              levels = c('5_meter.binary', 'street.binary'),
                              labels = c('5_meter', 'street'))) %>%
  select(-specificity) %>%
...
ggplot(data = granularity.analysis.data, aes(x = granularity, y = accuracy.value)) +
  geom_boxplot() +
  facet_grid(accuracy.type ~ label.type) +#, labeller = labeller(label.type = label.count.labeller)) +
  my.theme.discrete.x
```
