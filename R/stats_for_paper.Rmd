...
```{r turk.user.behavior.analysis.correlation.tests.1, echo=FALSE, fig.width=2, fig.height=2}
ggplot(data = turk.behavior.data %>% filter(accuracy.type == 'recall'), aes(x = labels.per.100m)) +
  geom_histogram(binwidth = 1,na.rm = TRUE) +
  theme_bw()

ggplot(data = turk.behavior.data %>% filter(accuracy.type == 'recall'), aes(x = meters.per.min)) +
  geom_histogram(binwidth = 10, na.rm = TRUE) +
  theme_bw()

ggplot(data = turk.behavior.data %>% filter(accuracy.type == 'recall'), aes(x = accuracy.value)) +
  geom_histogram(binwidth = 0.1, na.rm = TRUE) +
  xlab('Recall') +
  theme_bw()

ggplot(data = turk.behavior.data %>% filter(accuracy.type == 'precision'), aes(x = accuracy.value)) +
  geom_histogram(binwidth = 0.1, na.rm = TRUE) +
  xlab('Precision') +
  theme_bw()
```
...
```{r turk.user.behavior.analysis.correlation.tests.spearman, echo=FALSE, fig.width=2, fig.height=2}
turk.behavior.rec <- turk.behavior.data %>% filter(accuracy.type == 'recall')
turk.behavior.prec <- turk.behavior.data %>% filter(accuracy.type == 'precision')

freq.rec.test <- Kendall(turk.behavior.rec$labels.per.100m, turk.behavior.rec$accuracy.value)
speed.rec.test <- Kendall(turk.behavior.rec$meters.per.min, turk.behavior.rec$accuracy.value)
freq.prec.test <- Kendall(turk.behavior.prec$labels.per.100m, turk.behavior.prec$accuracy.value)
speed.prec.test <- Kendall(turk.behavior.prec$meters.per.min, turk.behavior.prec$accuracy.value)
freq.speed.test <- Kendall(turk.behavior.rec$labels.per.100m, turk.behavior.rec$meters.per.min)

first.var <- c('frequency', 'speed', 'frequency', 'speed', 'frequency')
second.var <- c('recall', 'recall', 'precision', 'precision', 'speed')
tau <- c(freq.rec.test$tau, speed.rec.test$tau, freq.prec.test$tau, speed.prec.test$tau, freq.speed.test$tau)
p.val <- c(freq.rec.test$sl[1], speed.rec.test$sl[1], freq.prec.test$sl[1], speed.prec.test$sl[1], freq.speed.test$sl[1])

kable(
  data.frame(first.var,
             second.var,
             tau = tau,
             p.value = format(round(p.val, digits = 5), scientific = FALSE, digits = 5)),
  align = 'l',
  digits = 3
)
```

We found the correlation between labeling frequency and recall (tau = `r format(freq.rec.test$tau, digits = 3)`) to be statistically significant (p = `r format(freq.rec.test$sl[1], scientific = FALSE, digits = 1)`). We also found the correlation between labeling frequency and auditing speed (tau = `r format(freq.speed.test$tau, digits = 3)`) to be statistically significant (p = `r format(freq.speed.test$sl[1], scientific = FALSE, digits = 1)`). There was not a significant correlation between either of those variables and precision, or between auditing speed and recall.
