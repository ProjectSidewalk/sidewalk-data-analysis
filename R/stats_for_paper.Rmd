...
dropoff.labels <- c('Finish tutorial',
                    ...
                    '9 missions',
                    '10 missions')
ggplot(aes(x = step, y = users.remaining), data = users.remaining.post.tutorial) +
  geom_line() +
  geom_point() +
...
ggplot(aes(x = step, y = users.remaining), data = users.remaining.post.tutorial.split) +
  geom_line(aes(colour = is.turker)) +
  geom_point(aes(colour = is.turker)) +
  scale_x_continuous(breaks = seq(0, 11),
                     labels = dropoff.labels,
...
# Create identical graphs to previous two sections.
# TODO create functions to generate each of these graphs to remove duplicate code.
ggplot(aes(x = step, y = users.remaining), data = users.remaining.all) +
  geom_line() +
  geom_point() +
  scale_x_continuous(breaks = seq(0, 19),
                     labels = combined.dropoff.labels,
                     expand = c(0.015, 0)) +
  scale_y_continuous(limits = c(0, NA), expand = c(0.015, 0)) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  theme(panel.grid.minor.x = element_blank())
ggplot(aes(x = step, y = users.remaining), data = users.remaining.all.split) +
  geom_line(aes(colour = is.turker)) +
  geom_point(aes(colour = is.turker)) +
  scale_x_continuous(breaks = seq(0, 19),
                     labels = combined.dropoff.labels,
