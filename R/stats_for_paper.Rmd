...
audit.length <-
  ...
  summarize(feet_audited = sum(feet_audited), audit_count = n()) %>%
...
kable(
  speeds %>%
    filter(labels_per_100m >= 3.75) %>%
    group_by(role) %>%
    summarise_at(
      vars(miles_audited, mission_count, audit_count, minutes_audited, minutes_per_1k_ft, label_count, labels_per_100m),
      median, na.rm = TRUE
      ),
  digits = 3
  )
...
kable(
  speeds %>%
    ...
    group_by(role) %>%
    summarise_at(
      vars(miles_audited, mission_count, audit_count, hours_audited, label_count),
      sum, na.rm = TRUE
      ),
  digits = 3
  )
