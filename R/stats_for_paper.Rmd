...
steps.by.ip <-
  ...
  dplyr::summarise(steps = first(steps),
                   ip.address = first(ip.address)) %>%
  group_by(ip.address) %>%
  dplyr::summarise(steps = max(steps))
)

get.users.remaining <- Vectorize(
  function(step) {
    nrow(steps.by.ip[steps.by.ip$steps >= step,])
  }
)
...
```{r public.deployment.tutorial.dropoff.plots, echo=FALSE, fig.width=7, fig.height=4}
