...
if (REFRESH_PUBLIC_DEPLOYMENT_DATA) {
  db.connection.public.deployment <- dbConnect(drv, dbname = "sidewalk-april",
...
descriptions.path <- '../data/stats_for_paper_descriptions.csv'
...
if (REFRESH_PUBLIC_DEPLOYMENT_DATA) {
  ...
  reg.descriptions <-
    dbGetQuery(
      db.connection.public.deployment,
      'SELECT DISTINCT audit_task.user_id, label.label_id, problem_description.description
      FROM label
      INNER JOIN problem_description ON problem_description.label_id = label.label_id
      ...
      WHERE audit_task.user_id <> \'97760883-8ef0-4309-9a5e-0c086ef27573\'
          AND label.gsv_panorama_id NOT IN (SELECT gsv_panorama_id FROM gsv_onboarding_pano);'
    )
  anon.descriptions <-
    dbGetQuery(
      db.connection.public.deployment,
      'SELECT DISTINCT audit_task_environment.ip_address AS user_id, label.label_id, problem_description.description
      FROM label
      INNER JOIN problem_description ON problem_description.label_id = label.label_id
      INNER JOIN audit_task ON audit_task.audit_task_id = label.audit_task_id
      INNER JOIN audit_task_environment ON audit_task_environment.audit_task_id = label.audit_task_id
      WHERE  audit_task.user_id = \'97760883-8ef0-4309-9a5e-0c086ef27573\'
          AND label.gsv_panorama_id NOT IN (SELECT gsv_panorama_id FROM gsv_onboarding_pano);'
    )
  reg.audits <-
  ...
  descriptions <- bind_rows(reg.descriptions, anon.descriptions)
  ...
  file.copy(descriptions.path, paste('../data/old/stats_for_paper_descriptions.csv', now, sep = '_'))
  ...
  write.csv(descriptions, descriptions.path, row.names = FALSE)
  ...
  descriptions <- read.csv(descriptions.path, colClasses = c('character', 'numeric', 'character'))
...
```{r public.deployment.how.much.data.unfiltered, echo=FALSE, warning=FALSE}
kable(
  non.tutorial.labels %>%
    dplyr::filter(user_id %in% user.list) %>%
    mutate(label_type = LABEL_TYPE_MAPPING[label_type_id]) %>%
...
nonfiltered.descriptions.data <-
  descriptions %>%
  group_by(user_id) %>%
  dplyr::summarise(n_descriptions = n()) %>%
  right_join(nonfiltered.user.behavior.data, by = 'user_id') %>%
  replace_na(list(n_descriptions = 0))
...
filtered.descriptions.data <-
  descriptions %>%
  group_by(user_id) %>%
  dplyr::summarise(n_descriptions = n()) %>%
  right_join(filtered.user.behavior.data, by = 'user_id') %>%
  replace_na(list(n_descriptions = 0))
...
description.aov.nonfilter <- aov(ihs(n_descriptions) ~ role, data = nonfiltered.descriptions.data)
...
kable(
  bind_rows(
    ...
    TukeyHSD(description.aov.nonfilter)[[1]] %>% data.frame() %>%
      tibble::rownames_to_column('comparison') %>% mutate(variable = 'n_descriptions')
  ) %>%
...
description.aov.filter <- aov(ihs(n_descriptions) ~ role, data = filtered.descriptions.data)
...
kable(
  bind_rows(
    ...
    base::summary(description.aov.filter)[[1]]['role',]
  ) %>%
    cbind(list(variable = c('mission_count', 'label_count', 'miles_per_hour', 'minutes_audited',
                            'tutorial_minutes', 'n_descriptions'))) %>%
    mutate(F = !!sym('F value'),
...
kable(
  bind_rows(
    ...
    TukeyHSD(tutorial.aov.filter)[[1]] %>% data.frame() %>%
      tibble::rownames_to_column('comparison') %>% mutate(variable = 'tutorial_minutes'),
    TukeyHSD(description.aov.filter)[[1]] %>% data.frame() %>%
      tibble::rownames_to_column('comparison') %>% mutate(variable = 'n_descriptions')
  ) %>%
    mutate(p = p.num.to.p.str.with.sig.code(p.adj)) %>%
    dplyr::select(variable, comparison, diff, p),
...
```{r turk.label.type.accuracy.glmm, echo=FALSE, message=FALSE}
type.test.data <-
  turk.showing.stats.by.label.type %>%
  dplyr::filter(granularity == 'street',
...
```{r turk.user.behavior.tests, echo=FALSE, message=FALSE}
