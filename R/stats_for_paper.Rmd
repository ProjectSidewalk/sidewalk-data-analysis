...
```{r turk.zone.type.distribution, echo=FALSE, fig.width=4, fig.height=3}
...
n.routes.by.zone.type <- conditions.w.semantic.zone %>% group_by(zone.type) %>% summarise(n = n())
route.count.zone.lookup <- setNames(n.routes.by.zone.type$n, n.routes.by.zone.type$zone.type)

total.gt.by.zone <- gt.count.semantic.zone %>% group_by(zone.type) %>% summarise(L = sum(gt.labels))
gt.label.count.zone.lookup <- setNames(total.gt.by.zone$L, total.gt.by.zone$zone.type)
```
...
```{r gt.label.dist.zone.plot, fig.height=1.5, fig.width=10, warning=FALSE, message=FALSE, echo=FALSE}
plot.gt.dist.for.zone.type <- function(zone.type.str, N, L) {
  ggplot(data = gt.count.semantic.zone %>% filter(zone.type == zone.type.str),
...
grid.arrange(plot.gt.dist.for.zone.type('Commercial',
                                        route.count.zone.lookup['Commercial'],
                                        gt.label.count.zone.lookup['Commercial']),
             plot.gt.dist.for.zone.type('Mixed-Use',
                                        route.count.zone.lookup['Mixed-Use'],
                                        gt.label.count.zone.lookup['Mixed-Use']),
             plot.gt.dist.for.zone.type('Residential Apt',
                                        route.count.zone.lookup['Residential Apt'],
                                        gt.label.count.zone.lookup['Residential Apt']),
             plot.gt.dist.for.zone.type('Residential House',
                                        route.count.zone.lookup['Residential House'],
                                        gt.label.count.zone.lookup['Residential House']),
             plot.gt.dist.for.zone.type('Unzoned',
                                        route.count.zone.lookup['Unzoned'],
                                        gt.label.count.zone.lookup['Unzoned']),
             ncol = 5)
...
x.labels.with.n <- n.routes.by.zone.type %>% mutate(lab = paste0(zone.type, ' (n=', n, ')')) %>% pull(lab)
...
ggplot(data = zone.type.analysis.data %>% filter(label.type == 'All'),
  ...
  scale_x_discrete(labels = x.labels.with.n) +
  facet_grid(accuracy.type ~ .) +
