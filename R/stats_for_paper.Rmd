...
speeds <-
  ...
  mutate(feet_per_min = feet_audited / auditing_time,
         minutes_per_mission = approximate.minutes.per.mission(auditing_time, mission_count, feet_audited))
         # minutes_per_mission = auditing_time / mission_count)
```
...
kable(
  speeds %>%
    group_by(role) %>%
    summarise_at(
      vars(miles_audited, mission_count, auditing_time, feet_per_min, minutes_per_mission),
      median, na.rm = TRUE
      ),
  digits = 4
  )
