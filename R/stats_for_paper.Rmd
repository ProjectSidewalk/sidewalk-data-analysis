...
steps.by.ip <-
  ...
  mutate(cur.time = min(time[action == 'start']),
         ...
         steps = if_else(has.step & has.end, 8, steps),
                            ...
                            cur.time)) %>%
  summarise_at(vars(steps, end.time), first) %>%
  summarise_at(vars(steps, ip.address, end.time), first) %>%
  group_by(ip.address) %>%
...
dropoffs.by.ip <-
  ...
  distinct() %>%
  rbind(filter(dropoffs, action == 'PanoId_Changed')) %>%
  inner_join(steps.by.ip, by = 'ip.address') %>%
  filter(steps.complete == 8) %>%
  group_by(ip.address) %>%
  filter((action == 'MissionComplete' & difftime(timestamp, end.timestamp) > 0)
         | (action == 'PanoId_Changed' & difftime(timestamp, end.timestamp, units = 'secs') > -1),
