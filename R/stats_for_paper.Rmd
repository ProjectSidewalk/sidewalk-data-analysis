...
format.tukey.table <- function(tukey, outcome.var, test.data, descending = FALSE) {
  ...
  ordering <-
    if(descending == TRUE) {
      outcome.by.type %>% arrange(desc(!!sym(outcome.var))) %>% pull(label.type)
    } else {
      outcome.by.type %>% arrange(!!sym(outcome.var)) %>% pull(label.type)
    }

  data.frame(comparison = names(tukey$test$pvalues),
             p.value = unname(tukey$test$pvalues),
             z.value = unname(tukey$test$tstat)) %>%
  mutate(comparison = if_else((descending == TRUE & z.value > 0) | (descending == FALSE & z.value < 0),
  ...
  right_join(outcome.by.type, by = c('first.type' = 'label.type')) %>%
  mutate(label.type = factor(first.type, levels = ordering)) %>%
  arrange(label.type) %>%
  dplyr::select(label.type, test, p.value, z.value, outcome.var)
}
```
...
```{r turk.label.type.recall.glmm, echo=FALSE}
type.test.data <-
  turk.showing.stats.by.label.type %>%
  filter(granularity == 'street',
...
Using likelihood ratio tests (LRTs), we found the contribution of the fixed effect (label type) to have a statistically significant association with recall (likelihood ratio = `r format(lrt.type.recall['label.type','LRT'], digits = 5)`, `r print.p.value(lrt.type.recall['label.type','Pr(Chi)'])`). We also found label type to have a statistically significant association with precision (likelihood ratio = `r format(lrt.type.prec['label.type','LRT'], digits = 5)`, `r print.p.value(lrt.type.prec['label.type','Pr(Chi)'])`).
...
```{r turk.label.type.recall.tukey, echo=FALSE}
# Extracts the info we want from the Tukey HSD, and formats it nicely for a table.
tukey.type.recall.clean <-
  format.tukey.table(tukey.type.recall, 'recall', type.test.data.recall, descending = TRUE)
tukey.type.prec.clean <-
  format.tukey.table(tukey.type.prec, 'precision', type.test.data.prec, descending = TRUE)
...
We also created a binomial mixed effects model to determine the relationship between severity and recall. We had severity (high or low) as the fixed effect and user id nested in route id as random effects. We modeled recall as binomial and used a logistic link function. Using a likelihood ratio test (LRT), we found the contribution of the fixed effect (severity) to be statistically significant (likelihood ratio = `r format(lrt.severity.2['included.severity','LRT'], digits = 5)`, `r print.p.value(lrt.severity.2['included.severity','Pr(Chi)'])`).
