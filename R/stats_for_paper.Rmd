...
low.severity.check.data <-
  ...
  filter(included.severity %in% c('all', '>=3', '>=4'),
         label.type %in% c('NoCurbRamp', 'Obstacle', 'SurfaceProb', 'Problem'),
         worker.type != 'turk0') %>%
  droplevels %>%
  mutate(included.severity = ordered(included.severity,
                                     labels = c('all', '>=3', '>=4'),
                                     levels = c('all', '>=3', '>=4')))
...
grouped.low.severity.data <-
  low.severity.check.data %>%
  filter(binary == TRUE) %>%
  group_by(label.type, worker.type, included.severity, granularity) %>%
...
kable(
  low.severity.check.data %>%
    filter(worker.type == 'turk1',
           binary == FALSE,
           granularity == 'street') %>%
    group_by(worker.type, label.type, included.severity) %>%
    summarize(gt.labels = sum(true.pos, false.neg)) %>%
    ungroup() %>%
    spread(label.type, gt.labels) %>%
    select(-worker.type),
...
ggplot(data = grouped.low.severity.data, aes(included.severity)) +
