...
dropoffs.by.ip <-
  ...
  mutate(steps.complete = case_when(missions.complete >= 2 ~ 3,
                                    missions.complete == 1 ~ 2,
                                    panos.changed >= 2     ~ 1,
                                    TRUE                   ~ 0))

users.remaining.dropoffs <-
  data.frame(steps.complete = seq(0, 3)) %>%
  mutate(users.remaining = get.users.remaining(dropoffs.by.ip, steps.complete))
