...
TODO: Put accuracy numbers in a separate table, and include f-measure. <br>
TODO: Come up with our own zone type descriptions, possibly aggregating as well. <br>
TODO: Add average IRR across all rounds (but separated by label type). <br>
...
turk.study.summary.stats <-
  ...
  dplyr::summarize_at(vars(total.recall:problem.precision), sum, na.rm = TRUE) %>%
  ungroup() %>%
  left_join(labels.turk.study, by = c('worker1' = 'user_id', 'condition.id' = 'condition_id')) %>%
  group_by_at(vars(worker1:problem.precision)) %>%
