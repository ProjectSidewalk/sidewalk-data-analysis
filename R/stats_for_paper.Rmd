...
important.average.stats <-
  nonfiltered.speeds %>%
  mutate(km = meters_audited / 1000) %>%
  rename(meters_per_min = m_per_min) %>%
  dplyr::select(role, minutes_audited, meters_per_min, minutes_per_session) %>%
...
kable(
  nonfiltered.speeds %>%
    ...
    rename(audits = audit_count,
           labels = label_count,
           missions = mission_count) %>%
    dplyr::select(role, km, missions, audits, labels, mult_sess) %>%
    summarise_vars('role', funcs = sum, include.n = TRUE) %>%
