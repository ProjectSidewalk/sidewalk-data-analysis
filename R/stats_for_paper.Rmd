...
if (REFRESH_TURK_STUDY_DATA) {
  ...
  turker.labels.turk.study <-
    dbGetQuery(
      db.connection.turk.study.production,
      'SELECT label_id, turker_id, label_type_id, audit_task.audit_task_id, condition_id
      FROM amt_assignment
...
missions.by.user.group <-
  ...
  summarize_at(vars(n.missions, distance.feet), sum) %>%
  mutate(distance.miles = distance.feet / 5280) %>%
  select(worker.type, n.missions, distance.miles)
```
