...
TODO: Add turk3 and turk5 to the user groups (3 and 5 turkers with majority vote, respectively). <br>
...
turk.study.summary.stats <-
  data.with.raw.accuracy %>%
  filter(n.workers == 1, !binary, !remove.low.severity, granularity == 'street') %>%
  filter(label.type %in% c('All', 'AllProb')) %>%
  ...
  dplyr::summarize_at(vars(total.recall:problem.f.measure), sum, na.rm = TRUE) %>%
  ungroup() %>%
  left_join(labels.turk.study, by = c('worker1' = 'user_id', 'condition.id' = 'condition_id')) %>%
  group_by_at(vars(worker1:problem.f.measure)) %>%
