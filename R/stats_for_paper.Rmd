...
if (REFRESH_PUBLIC_DEPLOYMENT_DATA) {
  ...
  tutorials <-
    dbGetQuery(
      db.connection.public.deployment,
      'SELECT action, ip_address, note, ips.audit_task_id, timestamp
      FROM
      (
          SELECT DISTINCT audit_task_interaction.audit_task_id, ip_address
          FROM audit_task_interaction
          INNER JOIN audit_task
              ON audit_task_interaction.audit_task_id = audit_task.audit_task_id
          INNER JOIN audit_task_environment
              ON audit_task.audit_task_id = audit_task_environment.audit_task_id
          WHERE task_start > \'2017-07-10 00:00:00.000+00\'
  ...
  dropoffs <-
    dbGetQuery(
      db.connection.public.deployment,
      'SELECT DISTINCT ips1.ip_address, action, timestamp
      FROM
      (
          SELECT DISTINCT audit_task_interaction.audit_task_id, ip_address
          FROM audit_task_interaction
      ...
      ON ips1.audit_task_id = ips2.audit_task_id
          AND ips1.ip_address = ips2.ip_address
      INNER JOIN audit_task_environment
          ON ips2.ip_address = audit_task_environment.ip_address
      INNER JOIN audit_task
          ON audit_task_environment.audit_task_id = audit_task.audit_task_id
      INNER JOIN audit_task_interaction
          ON audit_task.audit_task_id = audit_task_interaction.audit_task_id
      WHERE task_start > \'2017-07-10 00:00:00.000+00\'
          AND action IN (\'Onboarding_End\', \'MissionComplete\');'
    )
  ...
  tutorials <- read.csv(tutorials.path,
                        colClasses = c(replicate(3, 'character'), 'numeric', 'POSIXct'))
  dropoffs <- read.csv(dropoffs.path, colClasses = c('character', 'character', 'POSIXct'))
}
...
tasks.where.tutorial.started <-
  ...
  filter(has.transition == TRUE) %>%
  pull(audit.task.id)

suppressWarnings(
steps.by.ip <-
  tutorials.parsed %>%
  filter(audit.task.id %in% tasks.where.tutorial.started) %>%
  group_by(audit.task.id, ip.address) %>%
  mutate(cur.time = min(time[action == 'start']),
         ...
         steps = if_else(has.step & has.end, 8, steps),
                            ...
                            cur.time)) %>%
  dplyr::summarise(steps = first(steps),
                   end.time = first(end.time)) %>%
  dplyr::summarise_at(vars(steps, ip.address, end.time), first) %>%
  group_by(ip.address) %>%
...
get.users.remaining <- Vectorize(
  function(step) {
    nrow(steps.by.ip[steps.by.ip$steps.complete >= step,])
  }
)
users.remaining.by.step <-
  data.frame(step = seq(0, 8)) %>%
  mutate(users.remaining = get.users.remaining(step))

n.started.tutorial <- users.remaining.by.step[1,'users.remaining']
n.did.first.step.tutorial <- users.remaining.by.step[2,'users.remaining']
n.finished.tutorial <- users.remaining.by.step[9,'users.remaining']
...
Of the `r n.started.tutorial` people who started the tutorial, `r n.did.first.step.tutorial` finished the first step (`r format(percent.did.first.step.tutorial, digits = 0)`%), and `r n.finished.tutorial` completed the tutorial (`r format(percent.finished.tutorial, digits = 0)`%).

```{r public.deployment.tutorial.dropoff.plots, echo=FALSE, fig.width=5, fig.height=3}
ggplot(aes(x = step, y = users.remaining), data = users.remaining.by.step) +
  geom_line() +
  geom_point() +
  scale_x_continuous(breaks = seq(0, 8), expand = c(0.015, 0)) +
...
```{r public.deployment.user.dropoff, echo=FALSE}
...
missions.by.ip <-
  dropoffs %>%
  filter(action == 'MissionComplete') %>%
  inner_join(steps.by.ip, by = 'ip.address') %>%
  filter(steps.complete == 8) %>%
  group_by(ip.address) %>%
  filter(difftime(timestamp, end.timestamp) > 0,
         difftime(timestamp, end.timestamp, units = 'hours') < 0.5) %>%
  dplyr::summarise(missions.complete = n()) %>%
  right_join(steps.by.ip, by = 'ip.address') %>%
  filter(steps.complete == 8) %>%
  replace_na(list(missions.complete = 0)) %>%
  mutate(missions.complete = pmin(2, missions.complete))

get.users.remaining.drop <- Vectorize(
  function(missions) {
    nrow(missions.by.ip[missions.by.ip$missions.complete >= missions,])
  }
)
users.remaining.by.mission <-
  data.frame(missions.complete = seq(0, 2)) %>%
  mutate(users.remaining = get.users.remaining.drop(missions.complete))

n.did.one.mission <- users.remaining.by.mission[2,'users.remaining']
n.did.two.missions <- users.remaining.by.mission[3,'users.remaining']
...
Below we look at how users drop off after finishing the tutorial. For consistency with the tutorial analysis, we use IP address to denote a user (the `r n.finished.tutorial` IP addresses who finished the tutorial in the previous section are the same `r n.finished.tutorial` IPs that we are looking at below). As such, we are looking at only those who finished the tutorial after July 10th, 2017.
...
Of the `r n.finished.tutorial` people who finished the tutorial, `r n.did.one.mission` finished at least one mission afterwards (`r format(percent.did.one.mission, digits = 0)`%), and `r n.did.two.missions` completed at least two missions afterwards (`r format(percent.did.two.missions, digits = 0)`%).

```{r public.deployment.user.dropoff.plot, echo=FALSE, fig.width=5, fig.height=3}
ggplot(aes(x = missions.complete, y = users.remaining), data = users.remaining.by.mission) +
  geom_line() +
  geom_point() +
  scale_x_continuous(breaks = seq(0, 2), labels = c('0', '1', '>=2'), expand = c(0.015, 0)) +
  scale_y_continuous(limits = c(0, NA), expand = c(0.015, 0)) +
  theme_bw() +
  theme(panel.grid.minor.x = element_blank())
```
