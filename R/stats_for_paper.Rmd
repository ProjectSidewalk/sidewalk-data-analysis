...
turk.behavior.data <-
  ...
  ungroup() %>%
  left_join(times.turk.study, by = c('worker1' = 'user_id')) %>%
  mutate(distance.feet = if_else(is.anon.route, 2000, 4000),
         distance.meters = distance.feet / 3.28084,
         labels.per.100m = 100 * n.labels / distance.meters,
         meters.per.min = distance.meters / minutes_audited) %>%
  gather(key = accuracy.type, value = accuracy.value, recall, precision) %>%
  mutate(accuracy.type = factor(accuracy.type, levels = c('recall', 'precision'))) %>%
  filter(label.type %in% c('All', 'Problem'))
...
behavior.test.data.unscaled <-
  ...
  mutate(distance.feet = if_else(is.anon.route, 2000, 4000),  # intermediate value
         ...
         meters.per.min = distance.meters / minutes_audited,
         user.id = worker1,
         label.type = as.character(label.type)) %>%
  inner_join(rbind(avg.search.times.prob, avg.search.times.all),
...
all.coeffs <-
  ...
  mutate(association = case_when(Estimate > 0 & p < 0.05 ~ '+',
                                 ...
                                 TRUE                    ~ 'NA'),
         p.value = p.num.to.p.str.with.sig.code(p)) %>%
  dplyr::rename(estimate = Estimate, std.err = Std..Error) %>%
  select(accuracy.type, label.type, param, association, estimate, std.err, p.value, LRT)
