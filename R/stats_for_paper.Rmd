...
```{r turk.label.difficulty.analysis, echo=FALSE, fig.width=7, fig.height=4}
accuracy.by.label.type <-
  turk.showing.stats.by.label.type %>%
  filter(granularity == 'street') %>%
  group_by(label.type) %>%
...
distinct.turkers <- data.with.raw.accuracy %>% distinct(worker1) %>% pull()
...
ave.labeling.time.by.type <-
  ...
  filter(user_id %in% distinct.turkers) %>%
  ...
  group_by(label_type) %>%
  dplyr::summarise(median_sec_to_label = median(seconds_to_label),
                   mean_sec_to_label = mean(seconds_to_label)) %>%
  inner_join(accuracy.by.label.type, by = c('label_type' = 'label.type')) %>%
  arrange(median_sec_to_label)
...
```{r turk.label.difficulty.RMANOVA.assumption, echo=FALSE, fig.width=2, fig.height=2, message=FALSE}
labeling.times.with.type <-
  labeling.times.turk.study %>%
  filter(user_id %in% distinct.turkers) %>%
  mutate(label_type = factor(extract_label_type(note))) %>%
  mutate(label_type = fct_recode(label_type, 'SurfaceProb' = 'SurfaceProblem')) %>%
...
ggplot(labeling.times.with.type %>% filter(label_type == 'CurbRamp'), aes(x = seconds_to_label)) +
  geom_histogram() +
  ggtitle('CurbRamp') +
  theme_bw()
ggplot(labeling.times.with.type %>% filter(label_type == 'NoCurbRamp'), aes(x = seconds_to_label)) +
  geom_histogram() +
  ggtitle('NoCurbRamp') +
  theme_bw()
ggplot(labeling.times.with.type %>% filter(label_type == 'Obstacle'), aes(x = seconds_to_label)) +
  geom_histogram() +
  ggtitle('Obstacle') +
  theme_bw()
ggplot(labeling.times.with.type %>% filter(label_type == 'SurfaceProb'), aes(x = seconds_to_label)) +
  geom_histogram() +
  ggtitle('SurfaceProb') +
  theme_bw()

ggplot(labeling.times.with.type %>% filter(label_type == 'CurbRamp'), aes(x = log(seconds_to_label))) +
  geom_histogram() +
  ggtitle('CurbRamp') +
  theme_bw()
ggplot(labeling.times.with.type %>% filter(label_type == 'NoCurbRamp'), aes(x = log(seconds_to_label))) +
  geom_histogram() +
  ggtitle('NoCurbRamp') +
  theme_bw()
ggplot(labeling.times.with.type %>% filter(label_type == 'Obstacle'), aes(x = log(seconds_to_label))) +
  geom_histogram() +
  ggtitle('Obstacle') +
  theme_bw()
ggplot(labeling.times.with.type %>% filter(label_type == 'SurfaceProb'), aes(x = log(seconds_to_label))) +
  geom_histogram() +
  ggtitle('SurfaceProb') +
  theme_bw()
```
...
get.gt.count.distro <- function(data, recode.func) {
  data %>%
    ...
    ungroup() %>%
    select(label.type, zone.type, gt.labels)
}

plot.gt.dist.for.zone.type <- function(data, zone.type.str, N, L) {
  ggplot(data = data %>% filter(zone.type == zone.type.str),
         aes(x = label.type, y = gt.labels, fill = label.type)) +
    geom_col(position = 'dodge') +
    my.theme.discrete.x +
    scale_fill_manual(values = c('green', 'red', 'blue', 'orange'), guide = FALSE) +
    theme(axis.text.x = element_blank(),
          axis.title.x = element_blank(),
          axis.title.y = element_blank(),
          plot.title = element_text(size = 8)) +
    ggtitle(paste0(zone.type.str, '(N=', N, ';L=', L, ')')) +
...
```{r gt.label.dist.zone.plot.semantic, fig.height=1.5, fig.width=10, warning=FALSE, message=FALSE, echo=FALSE}
...
grid.arrange(plot.gt.dist.for.zone.type(gt.count.semantic.zone,
                                        'Commercial',
                                        route.count.zone.lookup.semantic['Commercial'],
                                        gt.label.count.zone.lookup.semantic['Commercial']),
             plot.gt.dist.for.zone.type(gt.count.semantic.zone,
                                        'Mixed-Use',
                                        route.count.zone.lookup.semantic['Mixed-Use'],
                                        gt.label.count.zone.lookup.semantic['Mixed-Use']),
             plot.gt.dist.for.zone.type(gt.count.semantic.zone,
                                        'Residential Apt',
                                        route.count.zone.lookup.semantic['Residential Apt'],
                                        gt.label.count.zone.lookup.semantic['Residential Apt']),
             plot.gt.dist.for.zone.type(gt.count.semantic.zone,
                                        'Residential House',
                                        route.count.zone.lookup.semantic['Residential House'],
                                        gt.label.count.zone.lookup.semantic['Residential House']),
             plot.gt.dist.for.zone.type(gt.count.semantic.zone,
                                        'Unzoned',
                                        route.count.zone.lookup.semantic['Unzoned'],
                                        gt.label.count.zone.lookup.semantic['Unzoned']),
             ncol = 5)

# Version of the plot where they are on the same scale
# ggplot(data = gt.count.semantic.zone,
#        aes(gt.count.semantic.zone = zone.type, y = gt.labels, fill = label.type)) +
#   geom_col(position = 'dodge') +
#   my.theme.discrete.x +
#   scale_fill_manual(values = c('green', 'red', 'blue', 'orange'), guide = FALSE) +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
...
```{r gt.label.dist.zone.plot.density, fig.height=2, fig.width=6, warning=FALSE, message=FALSE, echo=FALSE}
...
grid.arrange(plot.gt.dist.for.zone.type(gt.count.density.zone,
                                        'Low-Moderate Density',
                                        route.count.zone.lookup.density['Low-Moderate Density'],
                                        gt.label.count.zone.lookup.density['Low-Moderate Density']),
             plot.gt.dist.for.zone.type(gt.count.density.zone,
                                        'Medium-High Density',
                                        route.count.zone.lookup.density['Medium-High Density'],
                                        gt.label.count.zone.lookup.density['Medium-High Density']),
             ncol = 2)

# Version of the plot where they are on the same scale
# ggplot(data = gt.count.density.zone,
#        aes(gt.count.density.zone = zone.type, y = gt.labels, fill = label.type)) +
#   geom_col(position = 'dodge') +
#   my.theme.discrete.x +
#   scale_fill_manual(values = c('green', 'red', 'blue', 'orange'), guide = FALSE) +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
