...
label.counts <-
  ...
  dplyr::rename(label_count = label_id)
...
classes <- c('numeric', replicate(5, 'character'), 'numeric', 'numeric', 'logical', 'logical',
             'numeric', 'factor', 'character', replicate(8, 'numeric'))
...
all.types.data <-
  ...
  group_by(condition.id, worker1, worker2, worker3, worker4, worker5, n.workers, worker.thresh,
           binary, remove.low.severity, low.severity.thresh, granularity, is.turker) %>%
...
label.types <- c('All', 'AllProb', 'CurbRamp', 'NoCurbRamp', 'Obstacle', 'SurfaceProb', 'NoSidewalk', 'Problem')
...
TODO: Add street level high level results (maybe?). <br>
...
all.type.accuracy.data <-
  data.with.raw.accuracy %>%
  filter(n.workers == 1, binary, !remove.low.severity, granularity == '5_meter') %>%
  filter(label.type %in% c('All', 'AllProb')) %>%
  select(label.type, precision, recall, specificity, f.measure, raw.accuracy) %>%
  group_by(label.type) %>%
  summarize_at(vars(precision:raw.accuracy), median, na.rm = TRUE)
...
turk.study.summary.stats <-
  data.with.raw.accuracy %>%
  filter(worker.type != 'turk0', !binary, !remove.low.severity, granularity == 'street') %>%
  filter(label.type %in% c('All', 'AllProb')) %>%
  group_by(condition.id, worker1, is.anon.route, worker.type) %>%
  mutate(total.recall = if_else(label.type == 'All', recall, NaN),
         total.precision = if_else(label.type == 'All', precision, NaN),
         total.f.measure = if_else(label.type == 'All', f.measure, NaN),
         total.n = if_else(label.type == 'All', true.pos + false.pos, NaN),
         problem.recall = if_else(label.type == 'AllProb', recall, NaN),
         problem.precision = if_else(label.type == 'AllProb', precision, NaN),
         problem.f.measure = if_else(label.type == 'AllProb', f.measure, NaN),
         problem.n = if_else(label.type == 'AllProb', true.pos + false.pos, NaN)) %>%
  dplyr::summarize_at(vars(total.recall:problem.n), sum, na.rm = TRUE) %>%
  ungroup()
...
turk.study.summary.stats.with.labels <-
  ...
  group_by_at(vars(worker1:problem.n)) %>%
...
kable(
  turk.study.summary.stats %>%
    ...
    dplyr::summarize(
      total.recall = median(total.recall, na.rm = TRUE),
      total.precision = median(total.precision, na.rm = TRUE),
      total.f.measure = median(total.f.measure, na.rm = TRUE),
      total.n = median(total.n, na.rm = TRUE),
      problem.recall = median(problem.recall, na.rm = TRUE),
      problem.precision = median(problem.precision, na.rm = TRUE),
      problem.f.measure = median(problem.f.measure, na.rm = TRUE),
      problem.n = median(problem.n, na.rm = TRUE)
    ),
  digits = 3,
  align = 'l'
  )
...
kable(
  clean.agreement.data %>%
    ...
    dplyr::summarize(mean.kripp.alpha = mean(kripp.alpha),
                     median.kripp.alpha = median(kripp.alpha)),
  digits = 3,
  align = 'l'
  )
```
...
```{r turk.zone.type.analysis, echo=FALSE, fig.width=4.5, fig.height=6}
condition.land.use <-
  zoning.with.volunteer.type %>%
...
ggplot(data = zone.type.analysis.data %>% filter(label.type == 'AllProb'),
       aes(x = zone.type, y = accuracy.value)) +
  geom_boxplot() +
  stat_summary(fun.y = mean, geom = 'point', color = 'red', size = 1) +
  facet_grid(accuracy.type ~ .) +
  ggtitle('Zone Type Accuracy Across Problem Label Types') +
  my.theme.discrete.x +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  scale_y_continuous(expand = c(0.01, 0)) # removes weird white space at bottom of plot
```
