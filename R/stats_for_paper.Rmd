    ...
    dbGetQuery(
      ...
      (
          ...
          WHERE role.role = \'Turker\'
              AND task_start > \'2017-07-10 00:00:00.000+00\'
...
pre.100.labels <-
  ...
  dplyr::filter(task_start < '2017-12-10 07:44:54.288+00') %>%
...
step.labels <- c('Start tutorial',
                 ...
                 'Fifth curb ramp',
                 'Finish tutorial')
ggplot(aes(x = step, y = users.remaining), data = users.remaining.tutorial) +
  geom_line() +
  geom_point() +
...
ggplot(aes(x = step, y = users.remaining), data = users.remaining.tutorial.split) +
...
n.took.step <- users.remaining.dropoffs[2,'users.remaining']
n.did.one.mission <- users.remaining.dropoffs[3,'users.remaining']
n.did.two.missions <- users.remaining.dropoffs[4,'users.remaining']
n.did.ten.missions <- users.remaining.dropoffs[12,'users.remaining']
...
dropoff.labels <- c('Finish tutorial',
                    ...
                    '9 missions',
                    '10 missions')
ggplot(aes(x = step, y = users.remaining), data = users.remaining.dropoffs) +
  geom_line() +
  geom_point() +
  scale_x_continuous(breaks = seq(0, 11),
                     labels = dropoff.labels,
                     expand = c(0.015, 0)) +
  scale_y_continuous(limits = c(0, NA), expand = c(0.015, 0)) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  theme(panel.grid.minor.x = element_blank())
ggplot(aes(x = step, y = users.remaining), data = users.remaining.dropoffs.split) +
  geom_line(aes(colour = is.turker)) +
  geom_point(aes(colour = is.turker)) +
  scale_x_continuous(breaks = seq(0, 11),
                     labels = dropoff.labels,
