...
if (REFRESH_PUBLIC_DEPLOYMENT_DATA) {
  reg.labels <-
    dbGetQuery(
      ...
      WHERE label.deleted = FALSE
          ...
          AND gsv_panorama_id NOT IN (SELECT gsv_panorama_id FROM gsv_onboarding_pano)'
      )
  anon.labels <-
    dbGetQuery(
      db.connection.public.deployment,
  ...
  audits <- read.csv(audits.path, colClasses = c('character', 'character', replicate(3, 'numeric')))
...
if (REFRESH_TURK_STUDY_DATA) {
  ...
  # make sure to not use production-gt database!
...
n.labels.pre.filter <- n_distinct(labels$label_id)
n.labels.researcher <- n_distinct(labels[labels$user_id %in% researcher.ids,'label_id'])
n.labels.bad.non.researcher <- n_distinct(labels[labels$user_id %in% bad.non.researchers.ids,'label_id'])
...
kable(
  filtered.speeds %>%
    ...
    dplyr::summarise(
      ...
      minutes.audited = median(minutes_audited, na.rm = TRUE),
      minutes.audited.std = sd(minutes_audited, na.rm = TRUE),
      km.per.hr = median(km_per_hr, na.rm = TRUE),
      km.per.hr.std = sd(km_per_hr[km_per_hr != Inf], na.rm = TRUE),
      m.per.min = median(m_per_min, na.rm = TRUE),
      m.per.min.std = sd(m_per_min[m_per_min = Inf], na.rm = TRUE),
      minutes.per.1k.ft = median(minutes_per_1k_ft, na.rm = TRUE),
      minutes.per.1k.ft.std = sd(minutes_per_1k_ft, na.rm = TRUE),
      labels = median(label_count, na.rm = TRUE),
      label.per.100m = median(labels_per_100m, na.rm = TRUE),
      labels.per.100m.std = sd(labels_per_100m, na.rm = TRUE),
      sessions = median(n_sessions, na.rm = TRUE),
      mins.per.sess = median(minutes_per_session, na.rm = TRUE)
      ),
  digits = 3,
  align = 'l'
  )
...
kable(
  turk.showing.stats.by.label.type %>%
    filter(granularity == 'street') %>%
    group_by(label.type) %>%
    dplyr::summarize(recall.md = median(recall, na.rm = TRUE),
                     recall.mn = mean(recall, na.rm = TRUE),
                     recall.std = sd(recall, na.rm = TRUE),
                     prec.md = median(precision, na.rm = TRUE),
                     prec.mn = mean(precision, na.rm = TRUE),
                     prec.std = sd(precision, na.rm = TRUE),
                     f.md = median(f.measure, na.rm = TRUE),
                     f.mn = mean(f.measure, na.rm = TRUE),
                     f.std = sd(f.measure, na.rm = TRUE)),
  digits = 3,
  align = 'l'
  )
```
...
kable(
  turk.study.summary.stats.with.labels %>%
    ...
    dplyr::summarize(
      labs.p.100m = median(labels.per.100m, na.rm = TRUE),
      labs.p.100m = sd(labels.per.100m, na.rm = TRUE),
      km.p.hr = median(km.per.hr, na.rm = TRUE),
      km.p.hr.std = sd(km.per.hr, na.rm = TRUE),
      m.p.min = median(meters.per.min, na.rm = TRUE),
      m.p.min.std = sd(meters.per.min, na.rm = TRUE),
      mins.p.1k.ft = median(minutes.per.1k.ft, na.rm = TRUE),
      mins.p.1k.ft.std = sd(minutes.per.1k.ft, na.rm = TRUE),
      mins.audited = median(minutes_audited, na.rm = TRUE),
      mins.audited.std = sd(minutes_audited, na.rm = TRUE)
    ),
  digits = 3,
  align = 'l'
  )
...
ave.labeling.time.by.type.and.user <-
  ...
  filter(user_id %in% distinct.workers) %>%
  mutate(label_type = factor(extract_label_type(note))) %>%
  mutate(label_type = fct_recode(label_type, 'SurfaceProb' = 'SurfaceProblem')) %>%
  filter(label_type %in% LABEL_TYPES_TO_ANALYZE) %>%
  mutate(label_type = factor(label_type)) %>%  # drops filtered out levels
  mutate(log_sec = log(seconds_to_label)) %>%
  group_by(label_type, user_id) %>%
  dplyr::summarise(seconds_to_label = median(seconds_to_label))  # probs want to take log here

ave.labeling.time.by.type <-
  ave.labeling.time.by.type.and.user %>%
  dplyr::summarise(mean_sec_to_label = mean(seconds_to_label),  # across users, by label type
                   median_s_to_label = median(seconds_to_label),
                   sd_s_to_label = sd(seconds_to_label)) %>%
  mutate(label_type = as.character(label_type)) %>%
  inner_join(accuracy.by.label.type, by = c('label_type' = 'label.type')) %>%
  arrange(mean_sec_to_label)
...
n.ramp.users <- sum(ave.labeling.time.by.type.and.user$label_type == 'CurbRamp')
n.surface.prob.users <- sum(ave.labeling.time.by.type.and.user$label_type == 'NoCurbRamp')
```

Now we want to check if this ordering is statistically significant. We would normally do an ANOVA followed by Tukey's HSD post-hoc analysis to see if ordering is significant. Since each user's labeling time is recorded _for each label type_, our next idea would be to run a Repeated Measures ANOVA.
...
```{r turk.label.difficulty.RMANOVA.assumption, echo=FALSE, fig.width=2, fig.height=2, message=FALSE, warning=FALSE}
...
# summary(aov(seconds_to_label ~ label_type + Error(user_id),
#             data = ave.labeling.time.by.type.and.user))

pre.test.data <- turk.showing.stats.by.label.type %>%
  filter(granularity == 'street') %>%
  filter(label.type %in% c('CurbRamp', 'NoCurbRamp', 'Obstacle', 'SurfaceProb')) %>%
  mutate(label.type = as.character(label.type),
  ...
  inner_join(ave.labeling.time.by.type.and.user,
             by = c('worker1' = 'user_id', 'label.type' = 'label_type'))

labeling.time.test.data <-
  pre.test.data %>%
  select(worker1, condition.id, seconds_to_label, label.type) %>%
  na.omit() %>%
  mutate(worker1 = factor(worker1),
         label.type = factor(label.type))
recall.test.data <-
  pre.test.data %>%
  select(worker1, condition.id, recall, label.type) %>%
  na.omit() %>%
  mutate(worker1 = factor(worker1),
         label.type = factor(label.type))
precision.test.data <-
  pre.test.data %>%
  select(worker1, condition.id, precision, label.type) %>%
  na.omit() %>%
  mutate(worker1 = factor(worker1),
         label.type = factor(label.type))

# This models random intercepts... So it assumes the slope is the same for each user, but that
# they have different intercepts, i.e., their baseline level of accuracy is different, but the
# effect of label type is the same across users and routes
...
labeling.times.model <- lme(seconds_to_label ~ label.type,
              random = ~ 1 | condition.id / worker1,
              data = labeling.time.test.data)
# summary(glht(labeling.times.model, linfct = mcp(label.type = 'Tukey')), test = adjusted('holm'))

recall.model <- lme(recall ~ label.type,
                    random = ~ 1 | condition.id / worker1,
                    data = recall.test.data)
# summary(glht(recall.model, linfct = mcp(label.type = 'Tukey')), test = adjusted('holm'))

precision.model <- lme(precision ~ label.type,
                    random = ~ 1 | condition.id / worker1,
                    data = precision.test.data)
# summary(glht(precision.model, linfct = mcp(label.type = 'Tukey')), test = adjusted('holm'))



# y <- turk.showing.stats.by.label.type %>%
#   filter(granularity == 'street') %>%
#   filter(label.type == 'All') %>%
#   mutate(condition.id = factor(condition.id)) %>%
#   select(condition.id, worker1, recall, label.type) %>%
#   inner_join(ave.labeling.time.by.type.and.user,
#              by = c('worker1' = 'user_id', 'label.type' = 'label_type')) %>%
#   na.omit()
# 
# z <- turk.showing.stats.by.label.type %>%
#   filter(granularity == 'street') %>%
#   filter(label.type %in% c('CurbRamp', 'NoCurbRamp', 'Obstacle', 'SurfaceProb')) %>%
#   mutate(label.type = factor(label.type),
#          condition.id = factor(condition.id)) %>%
#   dplyr::select(condition.id, worker1, recall, label.type) %>%
#   na.omit()
# 
# anova(lme(recall ~ seconds_to_label + label.type, random = ~ 1 | worker1, data = x))
# 
# z %>% group_by(label.type) %>% dplyr::summarise_at(vars(recall), mean)
# anova(lme(recall ~ label.type, random = ~ 1 | worker1, data = z))
# summary(glht(lme(recall ~ label.type, random = ~ 1 | worker1, data = z),
...
# 
# # the actual stuff...
# anova(lme(seconds_to_label ~ label_type, random = ~ 1 | user_id,
#           data = ave.labeling.time.by.type.and.user))
# 
# # Pairwise comparisons (post-hoc test)
# summary(glht(lme(seconds_to_label ~ label_type,
#                  random = ~ 1 | user_id,
#                  data = ave.labeling.time.by.type.and.user),
#              linfct = mcp(label_type = "Tukey")),
#         test = adjusted("holm"))

# Checking assumptions:
# https://stats.stackexchange.com/questions/77891/checking-assumptions-lmer-lme-mixed-models-in-r
# Homogeneity of variance:
# plot(lme(seconds_to_label ~ label_type, random = ~ 1 | user_id,
#          data = ave.labeling.time.by.type.and.user))
# Normally distributed residuals:
# qqnorm(lme(seconds_to_label ~ label_type, random = ~ 1 | user_id,
#                     data = ave.labeling.time.by.type.and.user), ~ranef(., level=1))
kable(
  data.frame(
    'label type' = c('CurbRamp', 'Obstacle', 'NoCurbRamp', 'SurfaceProb'),
    'sec to label' = c(8.14, 10.10, 12.78, 13.77),
    'sig greater than CurbRamp' = c('-', '0.011*', '0.000***', '0.000***'),
    'sig gt Obstacle' = c('-', '-', '0.027*', '0.005**'),
    'sig gt NoCurbRamp' = c('-', '-', '-', '0.301')
  ),
  digits = 3,
  align = 'l'
)
kable(
  data.frame(
    'label type' = c('CurbRamp', 'NoCurbRamp', 'Obstacle', 'SurfaceProb'),
    'recall' = c(0.91, 0.76, 0.49, 0.34),
    'sig less than CurbRamp' = c('-', '0.007**', '0.000***', '0.000***'),
    'sig lt NoCurbRamp' = c('-', '-', '0.000***', '0.000**'),
    'sig lt Obstacle' = c('-', '-', '-', '0.015*')
  ),
  digits = 3,
  align = 'l'
)
kable(
  data.frame(
    'label type' = c('CurbRamp', 'SurfaceProblem', 'Obstacle', 'NoCurbRamp'),
    'sec to label' = c(0.95, 0.72, 0.45, 0.18),
    'sig less than CurbRamp' = c('-', '0.000****', '0.000***', '0.000***'),
    'sig lt SurfaceProblem' = c('-', '-', '0.000***', '0.000***'),
    'sig lt Obstacle' = c('-', '-', '-', '0.000***')
  ),
  digits = 3,
  align = 'l'
)
...
get.zone.distro <- function(data, recode.func) {
  data %>%
    mutate(volunteer.type = factor(is.anon.route,
                                   levels = c(FALSE, TRUE),
                                   labels = c('Reg User Routes', 'Anon User Routes'))) %>%
...
gt.count.by.condition.and.label.type <-
  data.with.raw.accuracy %>%
  filter(worker.type %in% c('anon', 'reg'),
         binary == FALSE,
         included.severity == 'all',
         granularity == 'street',
         label.type %in% c('CurbRamp', 'NoCurbRamp', 'Obstacle', 'SurfaceProb')) %>%
...
get.gt.count.distro <- function(data, recode.func) {
  data %>%
    ...
    inner_join(gt.count.by.condition.and.label.type, by = 'condition.id') %>%
    group_by(label.type, zone.type) %>%
    dplyr::summarise(gt.labels = sum(gt.labels),
                     meters = sum(meters)) %>%
    ungroup() %>%
...
plot.gt.dist.for.zone.type <- function(data, n.data, l.data) {
  xlabs <-
    data %>%
    distinct(zone.type) %>%
    mutate(lab = paste0(zone.type, ' (N=', n.data[zone.type], ';L=', l.data[zone.type], ')')) %>%
    pull(lab)
...
zone.distro.by.volunteer.type <- get.zone.distro(conditions.w.semantic.zone, recode.semantic)

ggplot(data = zone.distro.by.volunteer.type,
...
x.labels.with.n.semantic <-
  ...
  mutate(lab = paste0(zone.type, ' (N=', N, ';L=', L, ')')) %>%
  pull(lab)
...
zone.distro.by.volunteer.type <- get.zone.distro(conditions.w.density.zone, recode.density)
...
```{r turk.user.behavior.analysis.correlation.tests.fits, echo=FALSE, fig.width=3, fig.height=4.5}
# Subset the data appropriately.
turk.behavior.corr.test.data <-
  turk.behavior.data %>%
  filter(label.type == 'All', accuracy.type != 'f.measure')
turk.behavior.rec <- turk.behavior.corr.test.data %>% filter(accuracy.type == 'recall')
turk.behavior.prec <- turk.behavior.corr.test.data %>% filter(accuracy.type == 'precision')

# Plot relationships between variables.
ggplot(turk.behavior.corr.test.data, aes(x = labels.per.100m, y = accuracy.value)) +
  geom_point() +
  stat_smooth(method = 'lm') +
  theme_bw() +
  facet_grid(accuracy.type ~ .)
ggplot(turk.behavior.corr.test.data, aes(x = meters.per.min, y = accuracy.value)) +
  geom_point() +
  stat_smooth(method = 'lm') +
  theme_bw() +
  facet_grid(accuracy.type ~ .)
...
```{r turk.user.behavior.analysis.correlation.tests, echo=FALSE, fig.width=2.5, fig.height=5}
# Kendall rank correlation coefficients.
freq.rec.k <- Kendall(turk.behavior.rec$labels.per.100m, turk.behavior.rec$accuracy.value)
speed.rec.k <- Kendall(turk.behavior.rec$meters.per.min, turk.behavior.rec$accuracy.value)
freq.prec.k <- Kendall(turk.behavior.prec$labels.per.100m, turk.behavior.prec$accuracy.value)
speed.prec.k <- Kendall(turk.behavior.prec$meters.per.min, turk.behavior.prec$accuracy.value)
freq.speed.k <- Kendall(turk.behavior.rec$labels.per.100m, turk.behavior.rec$meters.per.min)

# Note: cor.test defaults to Pearson coefficient.
freq.rec.p <- cor.test(turk.behavior.rec$labels.per.100m, turk.behavior.rec$accuracy.value)
speed.rec.p <- cor.test(turk.behavior.rec$meters.per.min, turk.behavior.rec$accuracy.value)
freq.prec.p <- cor.test(turk.behavior.prec$labels.per.100m, turk.behavior.prec$accuracy.value)
speed.prec.p <- cor.test(turk.behavior.prec$meters.per.min, turk.behavior.prec$accuracy.value)
freq.speed.p <- cor.test(turk.behavior.rec$labels.per.100m, turk.behavior.rec$meters.per.min)

kendall.list <- list(freq.rec.k, speed.rec.k, freq.prec.k, speed.prec.k, freq.speed.k)
pearson.list <- list(freq.rec.p, speed.rec.p, freq.prec.p, speed.prec.p, freq.speed.p)

first.var <- c('frequency', 'speed', 'frequency', 'speed', 'frequency')
second.var <- c('recall', 'recall', 'precision', 'precision', 'speed')
r <- unlist(lapply(pearson.list, function(x) x$estimate))
p.val.r <- unlist(lapply(pearson.list, function(x) x$p.value))
tau <- unlist(lapply(kendall.list, function(x) x$tau))
p.val.tau <- unlist(lapply(kendall.list, function(x) x$sl[1]))

kable(
  data.frame(first.var,
             second.var,
             r = r,
             p.value.r = format(round(p.val.r, digits = 5), scientific = FALSE, digits = 5),
             tau = tau,
             p.value.tau = format(round(p.val.tau, digits = 5), scientific = FALSE, digits = 5)),
  align = 'l',
  digits = 3
)
```

We found the correlation between labeling frequency and recall (r = `r format(freq.rec.k$tau, digits = 3)`) to be statistically significant (p = `r format(round(freq.rec.k$sl[1], digits = 5), scientific = FALSE, digits = 5)`). We also found the correlation between labeling frequency and auditing speed (r = `r format(freq.speed.k$tau, digits = 3)`) to be statistically significant (p = `r format(round(freq.speed.k$sl[1], digits = 5), scientific = FALSE, digits = 5)`). There was not a significant correlation between either of those variables and precision, or between auditing speed and recall.
...
_For those interested_, here is a look at the normality of the relevant variables in the tests above. As mentioned above, we don't use the Pearson's r because these variables have outliers. They also (sort of) don't meet the normality assumption. It is sort of a toss up for these variables in terms of normality, honestly. Below are histograms for them, and qq plots (which compares quantiles in the sample to a theoretical normal distribution, so values that trail off the line on the ends indicate non-normality).

```{r turk.user.behavior.analysis.correlation.normality, echo=FALSE, fig.width=2.5, fig.height=2.5}
# Reference on checking normality: http://www.sthda.com/english/wiki/normality-test-in-r

# Recall histogram.
ggplot(data = turk.behavior.rec,
       aes(x = accuracy.value)) +
  geom_histogram(binwidth = 0.1, na.rm = TRUE) +
  xlab('Recall') +
  theme_bw()
# Labeling frequency histogram.
ggplot(data = turk.behavior.rec,
       aes(x = labels.per.100m)) +
  geom_histogram(binwidth = 1,na.rm = TRUE) +
  theme_bw()
# Auditing speed histogram.
ggplot(data = turk.behavior.rec,
       aes(x = meters.per.min)) +
  geom_histogram(binwidth = 10,na.rm = TRUE) +
  theme_bw()

# Recall qq plot.
ggplot(data = turk.behavior.rec,
       aes(sample = accuracy.value)) +
  stat_qq(na.rm = TRUE) +
  geom_qq_line(na.rm = TRUE) +
  xlab('Theoretical normal') +
  ylab('Recall') +
  theme_bw()
# Labeling frequency qq plot.
ggplot(data = turk.behavior.rec,
       aes(sample = labels.per.100m)) +
  stat_qq(na.rm = TRUE) +
  geom_qq_line(na.rm = TRUE) +
  xlab('Theoretical normal') +
  ylab('Labels per 100m') +
  theme_bw()
# Auditing speed qq plot.
ggplot(data = turk.behavior.rec,
       aes(sample = meters.per.min)) +
  stat_qq(na.rm = TRUE) +
  geom_qq_line(na.rm = TRUE) +
  xlab('Theoretical normal') +
  ylab('Meters per minute') +
  theme_bw()

# Shapiro normality tests for labeling frequency and recall.
normality.shapiro.recall <- shapiro.test(turk.behavior.rec %>% pull(accuracy.value))
normality.shapiro.labeling.freq <- shapiro.test(turk.behavior.rec %>% pull(labels.per.100m))
normality.shapiro.auditing.speed <- shapiro.test(turk.behavior.rec %>% pull(meters.per.min))
```
...
recall.by.severity <-
  ...
  summarize(n.users = sum(!is.na(recall)),
            ...
            std.recall = sd(recall, na.rm = TRUE))
...
severity.test <- t.test(recall ~ included.severity,
                        data = low.severity.check.data %>%
                          filter(binary == TRUE, included.severity != 'all') %>%
                          droplevels)

```

Below is a table showing the average recall across all users for labels that had severity <=3 (in the ground truth) and labels that had severity >=4, along with the number of labels that fall into each of those categories. We also ran a two sample t-test with `r format(as.numeric(severity.test$parameter), digit = 2)` degrees of freedom, where the null hypothesis is that the average (mean) recall for the low severity labels is equal to the average recall for the high severity labels. We got a p-values of `r format(severity.test$p.value, digits = 2)`. Thus, with an alpha level of 0.05, we fail to reject the null hypothesis, and _cannot_ conclude that the means are different.
...
severity.test <- t.test(recall ~ included.severity,
                        data = low.severity.check.data.2 %>%
                          filter(binary == TRUE, included.severity != 'all') %>%
                          droplevels)

```

Below is a table showing the average recall across all users for labels that had severity <=2 (in the ground truth) and labels that had severity >=3, along with the number of labels that fall into each of those categories. We also ran a two sample t-test with `r format(as.numeric(severity.test$parameter), digit = 2)` degrees of freedom, where the null hypothesis is that the average (mean) recall for the low severity labels is equal to the average recall for the high severity labels. We got a p-values of `r format(severity.test$p.value, digits = 2)`. Thus, with an alpha level of 0.05, we reject the null hypothesis, and conclude that the means are in fact different. And we can see that the high severity recall is higher than the low severity recall, which matches our intuition.
