...
TODO: Add table of label counts, split by label type for rows and by user group for columns. Include totals and percentages.
...
all.type.accuracy.data <-
  data.with.raw.accuracy %>%
  filter(n.workers == 1, binary, !remove.low.severity) %>%
  filter(label.type %in% c('All', 'AllProb')) %>%
  select_at(vars(label.type, granularity, precision:f.measure, raw.accuracy)) %>%
...
turk.study.summary.stats <-
  data.with.raw.accuracy %>%
  filter(worker.type != 'turk0', !binary, !remove.low.severity) %>%
  filter(label.type %in% c('All', 'AllProb')) %>%
  ...
  mutate(all.recall = if_else(label.type == 'All', recall, NaN),
         ...
         all.n = if_else(label.type == 'All', true.pos + false.pos, NaN),
         prob.recall = if_else(label.type == 'AllProb', recall, NaN),
         prob.precision = if_else(label.type == 'AllProb', precision, NaN),
         prob.f.measure = if_else(label.type == 'AllProb', f.measure, NaN),
         prob.n = if_else(label.type == 'AllProb', true.pos + false.pos, NaN)) %>%
  dplyr::summarize_at(vars(all.recall:prob.n), sum, na.rm = TRUE) %>%
  ungroup()
...
turk.study.summary.stats.with.labels <-
  ...
  group_by_at(vars(worker1:prob.n)) %>%
...
turk.showing.stats.4.and.5 <- 
  turk.study.summary.stats %>%
    ...
    dplyr::summarize(
      ...
      prob.n = median(prob.n, na.rm = TRUE)
    ) %>%
...
NOTE: The "n" labels in this graph mark the number of _users_ (since one user = one observation in this graph).
...
ggplot(data = granularity.analysis.data, aes(x = granularity, y = accuracy.value)) +
  ...
  geom_text(data = granularity.analysis.labels,
...
kable(
  low.severity.check.data %>%
    filter(worker.type == 'turk1',
           binary == FALSE,
           granularity == 'street') %>%
    group_by(worker.type, label.type, low.severity.thresh) %>%
    summarize(gt.labels = sum(true.pos, false.neg)) %>%
    ungroup() %>%
    spread(label.type, gt.labels) %>%
    select(-worker.type),
...
ggplot(data = grouped.low.severity.data, aes(low.severity.thresh)) +
  geom_line(aes(y = recall, colour = granularity, group = granularity)) +
  geom_point(aes(y = recall, colour = granularity), size = 1) +
