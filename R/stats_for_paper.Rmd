...
ave.labeling.time.by.type.and.user <-
  ...
  mutate(label.type = factor(label.type)) %>%  # drops filtered out levels
  group_by(label.type, user.id) %>%
  dplyr::summarise(seconds.to.label = median(seconds.to.label)) %>%
...
log.labeling.time.model.base <- update(log.labeling.time.model, fixed = log.seconds.to.label ~ 1)
lrt.labeling.time <- anova(log.labeling.time.model.base, log.labeling.time.model)
```

Using a likelihood ratio test, we find the contribution by the fixed effect (label type) to be statistically significant (Likelihood ratio = `r format(lrt.labeling.time['log.labeling.time.model','L.Ratio'], digits = 5)`, `r print.p.value(lrt.labeling.time['log.labeling.time.model','p-value'])`), i.e., the association between label type and labeling time is statistically significant. To test that the ordering of the label types are statistically significant (e.g., that CurbRamp labeling time is significantly lower than Obstacle labeling time, etc), we do a post-hoc Tukey's HSD test. This essentially gives us a pairwise test between each label type, which lets us determine what parts of the ordering are significant. The results of which are shown in a table below.
...
# Makes a vector with the label types in order of largest to smallest labeling times.
labeling.time.label.type.ordering <-
  labeling.time.test.data %>%
  group_by(label.type) %>%
  summarize(log.seconds.to.label = mean(log.seconds.to.label)) %>%
  arrange(log.seconds.to.label) %>%
  pull(label.type) %>%
  as.character()

# Extracts the info we want from the Tukey HSD, and formats it nicely for a table.
tukey.log.labeling.time.clean <-
  data.frame(comparison = names(tukey.log.labeling.time$test$pvalues),
             p.value = unname(tukey.log.labeling.time$test$pvalues),
             z.value = unname(tukey.log.labeling.time$test$tstat)) %>%
  mutate(comparison = if_else(z.value < 0,
                              gsub("([[:alpha:]]+) - ([[:alpha:]]+)", "\\2 - \\1", comparison),
                              as.character(comparison)),
         first.type = gsub("([[:alpha:]]+) - ([[:alpha:]]+)", "\\1", comparison),
         second.type = gsub("([[:alpha:]]+) - ([[:alpha:]]+)", "\\2", comparison),
         first.index = match(first.type, labeling.time.label.type.ordering),
         second.index = match(second.type, labeling.time.label.type.ordering),
         test = paste('>', second.type),
         p.value = p.num.to.p.str.with.sig.code(p.value),
         z.value = format(abs(z.value), digits = 4, scientific = FALSE)) %>%
  filter(first.index - second.index == 1) %>%
  right_join(ave.labeling.time.by.type, by = c('first.type' = 'label.type')) %>%
  mutate(label.type = factor(first.type, levels = labeling.time.label.type.ordering),
         seconds.to.label = median.s.to.label,
         log.seconds.to.label = log(seconds.to.label)) %>%
  arrange(label.type) %>%
  dplyr::select(label.type, test, p.value, z.value, seconds.to.label, log.seconds.to.label)
